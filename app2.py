...
if uploaded_file:
    ...
    # Xử lý hàng tặng
    gift_column = next((col for col in df.columns if "Loại hàng" in col), None)
    if gift_column:
        df.loc[df[gift_column] == "Tặng", [col for col in df.columns if any(x in col for x in ["Doanh thu", "Chi phí Kinh Doanh", "Phí sàn"])]] = 0
